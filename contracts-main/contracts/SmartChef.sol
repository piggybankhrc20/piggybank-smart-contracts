...
// added functionnalities by the piggybankone's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when COINK mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 coinkReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake COINK tokens to TheBushV1
    ...
    // Withdraw COINK tokens from STAKING.
