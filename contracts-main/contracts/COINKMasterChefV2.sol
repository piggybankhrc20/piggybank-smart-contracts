...
// MasterChefV2 is the master of Coink. He can make Coink and he is a fair guy.
//
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    function pendingCoink(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accCoinkPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                safeCoinkTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function safeCoinkTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > coinkBal) {
            transferSuccess = coink.transfer(_to, coinkBal);
        } else {
            transferSuccess = coink.transfer(_to, _amount);
        }
		
        require(transferSuccess, "safeCoinkTransfer: transfer failed");
    }
    ...
    //Piggybankone has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _coinkPerBlock) public onlyOwner {
        massUpdatePools();
        coinkPerBlock = _coinkPerBlock;
		
		emit UpdateEmissionRate(msg.sender, _coinkPerBlock);
    }
}
